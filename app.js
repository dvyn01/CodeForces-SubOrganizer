...
function addSubmission(submission) {
    ...
    request('https://codeforces.com/contest/' + contestID + '/submission/' + submissionID, (error, response, body) => {
        if (!error && response.statusCode == 200) {
            ...
            // var wStream = fs.createWriteStream(`Codeforces Submissions - ${handle}/` + contestID + '/' + contestID + '-' + index + '-' + problemName + extension);
            // wStream.write(content, (error) => {
            //     if(error) {
            //         console.log(error);
            //     }
            //     else {
...
app.post('/', (req, res) => {
    ...
    request('http://codeforces.com/api/user.status?handle=' + handle + '&from=1&count=2000', (error, response, body) => {
        if (!error && response.statusCode == 200) {
            ...
            // var repeat = addSubmission(acceptedSubmissions(count));
            // setInterval(addSubmission, 5000, acceptedSubmissions[count]);
            // qall.qSyncAll(addSubmission, acceptedSubmissions, acceptedSubmissions.length).then(() => {
            //     console.log('done');
            // });
